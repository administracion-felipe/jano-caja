...
export default function CobroCaja() {
  ...
  async function abrirCaja() {
    ...
    const { data, error } = await supabase.from('caja_sesiones')
      .insert({ cajero: cajero.trim(), fondo_inicial: Number(fondo) || 0 }).select().single();
  ...
  async function cerrarCaja() {
    ...
    if (error) return setMsg({ tipo: 'error', txt: error.message });
    setSesion(null); setCobros([]); setCerrando(false); setArqueo(''); setCajero(''); setFondo('');
  }
    ...
    <div className="jc-bar">
      ...
      {sesion && (
        <div className="jc-session">
          <span className="jc-badge">Caja abierta</span>
          <span className="jc-cajero">{sesion.cajero}</span>
    ...
    return (
      <div className="jc"><style>{CSS}</style><div className="jc-wrap">
        ...
        <div className="jc-panel jc-open">
          <h2>Abrir caja</h2>
          <label className="jc-lbl">Cajero</label>
          <input className="jc-input" value={cajero} onChange={(e) => setCajero(e.target.value)} placeholder="Nombre" />
          <label className="jc-lbl">Fondo inicial</label>
          <input className="jc-input" type="number" value={fondo} onChange={(e) => setFondo(e.target.value)} placeholder="0" />
